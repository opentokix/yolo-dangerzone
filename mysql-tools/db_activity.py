...
def main():
    ...
    for i in range(1, num):
        if i % (num / 10) == 0:   # Will write a progress bar each
                                # 1/10th of the progress
            progress = "%d.." % (i)
            sys.stdout.write(progress)
            sys.stdout.flush()  # Flushing on each iteration, to make
# sure it goes to console

        cursor = connection.cursor()

        random_data2 = phrase(1)    # Getting a word

        for attempt_number in range(3):     # Doing x attempts before
                                            # throwing error
            try:

                cursor.execute("INSERT INTO test(data, data2) VALUES (%s, %s)",
                               (random.randint(1, 9), random_data2))
                connection.commit()
                break
            except MySQLdb.Error, e:
                print "MySQL Error %d: %s" % (e.args[0], e.args[1])

        time.sleep(.5)  # Sleeping for half a second before each insert,
